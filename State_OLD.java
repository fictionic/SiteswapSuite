...
public class State {
    private List<HandState> ipart;
    private List<HandState> fpart;
	...
	this.ipart = new ArrayList<HandState>();
	this.numBalls = 0;
	this.shift = 0;
	...
	for(int h=0; h<ss.numHands(); h++) {
	    this.ipart.add(new HandState());
	}
	//emulate juggling the pattern, adding balls to this.ipart at beats when needed
	//turn all of ss's negative tosses into antitosses
	ss.antitossify();
	int debugCounter = 0;
	do {
	    for(int b=0; b<ss.period(); b++) {
		printf("state: " + this.ipart);
		printf("curBeat: " + b);
		//loop through this.ipart
		for(int h=0; h<ss.numHands(); h++) {
		    printf("\tcurHand: " + h);
	...
	this.ipart = new ArrayList<HandState>();
	this.fpart = new ArrayList<HandState>();
	for(int i=0; i<numHands; i++) {
	    this.ipart.add(new HandState());	
	    this.fpart.add(new HandState());	
	}
    }

    public State(String stateString) {
	String validStateString = "(-?(\\d|a-z))+|\\[(-?(\\d|a-z))+(,(-?(\\d|a-z))+)*\\]";
	...
	if(!stateString.substring(0,1).equals("[")) {
	    this.ipart.add(new HandState(stateString));
	} else {
		...
		this.ipart.add(new HandState(stateString.substring(i,j)));
	    ...
	    this.ipart.add(new HandState(stateString.substring(i, stateString.length() - 1)));
	}
    }
	...
	return this.ipart.get(0).length;
    }
	...
	try {
	    this.ipart.get(handIndex).incrementValue(beatsFromNow);
	...
	try {
	    this.ipart.get(handIndex).decrementValue(beatsFromNow);
	} catch(ArrayIndexOutOfBoundsException e) {
	    System.out.println("handIndex cannot be greater than numHands");
	}
    }
	...
	for(HandState h : this.ipart) {
	    h.padWithOneZero();
	}
    }

    public Integer nowValue(int handIndex) {
	return this.ipart.get(handIndex).nowNode.value;
    }

    public Integer valueAtBeat(int handIndex, int beatsFromNow) {
	return this.ipart.get(handIndex).valueAtBeat(beatsFromNow);
    }
...
st1 is aligned with st2 if and only if diffSum == shiftSum
	...
	int diffSum = 0;
	//calculate sums
	for(int h=0; h<this.ipart.size(); h++) {
	    HandState.Node node1 = this.ipart.get(h).nowNode;
	    HandState.Node node2 = otherState.this.ipart.get(h).nowNode;
	...
	for(int h=0; h<this.ipart.size(); h++) {
	    if(!this.ipart.get(h).isAlignedWith(otherState.this.ipart.get(h))) {
		return false;
	    }
	}
	return true;
	...
	// so we can just take this.ipart.get(0).length as the length of the state)
	State longer = state1;
	...
	//determine which this.iparttate is the longest
	int longestLength = this.ipart.get(0).length;
	int i;
	for(i=1; i<this.ipart.size(); i++) {
	    if(this.ipart.get(i).length > longestLength) {
		longestLength = this.ipart.get(i).length;
	    }
	}
	//pad all this.iparttates with the appropriate number of zeroes
	for(i=0; i<this.ipart.size(); i++) {
	    for(int j=this.ipart.get(i).length; j<longestLength; j++) {
		this.ipart.get(i).padWithOneZero();
	    }
	}
    }

    public void throwBall(int fromHand, int height, int destHand) {
    }

    public void throwAntiball(int fromHand, int height, int destHand) {
    }

    public void throwBallToInfinity(int fromHand) {
    }

    public void throwAntiballToInfinity(int fromHand) {
    }

    public void catchBallFromInfinity(int atHand) {
    }

    public void catchAntiballFromInfinity(int atHand) {
    }

    public void throwBall(int fromHand, int height, boolean isInfinite, int destHand, boolean isAntiToss) {
	this.ipart.get(fromHand).throwBall(height, isInfinite, destHand, isAntiToss);
	...
	if(!isInfinite) {
	    ...
	    this.ipart.get(destHand).alterValueByOne(height, !isAntiToss);
	}
    }
	...
	for(int h=0; h<numHands(); h++) {
	    newHands.add(this.ipart.get(h).deepCopy());
	}
	out.this.ipart = newHands;
	return out;
    }
	...
	return this.ipart.toString();
    }

    public boolean equalsUpTo(State otherState, int shift) {
	if(this.ipart.size() != otherState.this.ipart.size()) {
	    return false;
	}
	for(int h=0; h<this.ipart.size(); h++) {
	    if(!this.ipart.get(h).equalsUpTo(otherState.this.ipart.get(h), shift)) {
		return false;
	    }
	}
	return true;
	...
	private HandState() {
	    this.nowNode = new Node(0, null);
	    this.lastNode = nowNode;
	    this.length = 1;
	}

	private HandState(Node newNowNode, int newLength) {
	...
	private HandState(String string) {
	    ...
	    this.nowNode = new Node(Integer.parseInt(curToken), null);
	    this.length = 1;
	    Node node = nowNode; //to keep track of which node we've just added, and add nodes off of it
		...
		Node newNode = new Node(Integer.parseInt(curToken), null);
	...
	private void incrementValueAtBeat(int beatsFromNow) {
	    int b=0;
	    Node n = this.nowNode;
	    while(b < beatsFromNow) {
		n = n.prev;
	    }
	    this.nowNode.increment();
	}
	...
	private void padWithOneZero() {
	    Node newLastNode = new Node(0, null);
	...
	private void shiftForward() {
	    Node newNowNode = new Node(null, nowNode);
	    nowNode = newNowNode;
	    length++;
	}

	private void throwBallToInfinity() {
	}

	private void throwAntiballToInfinity() {
	}

	private void catchBallFromInfinity() {
	}

	private void catchAntiballFromInfinity() {
	}
	...
	private boolean isAlignedWith(HandState otherHandState) {
	    Node node = nowNode;
	    Node node2 = otherHandState.nowNode;
	    //loop through beats
	    while(node != null) {
	...
	private class Node {
	    ...
	    private Node(Integer value, Node prevNode) {
		this.value = value;
		...
		Node toReturn = new Node(new Integer(value), newPrevNode);
		return toReturn;
	    } 
